...
;; Ben Bitdiddle order evaluation test
...
;; If the interpreter uses Applicative-order => the test won't finish since in this behavior, the interpreter 
;; evaluate each sub expression before applying the operation, so it will tend to evaluate `p` indefinitely
